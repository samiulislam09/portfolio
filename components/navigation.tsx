...
export function Navigation() {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "md:hidden bg-black fixed inset-0 top-16 z-50 transition-all duration-300 ease-in-out bg-background/95 backdrop-blur-sm",
          isOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-full pointer-events-none"
        )}
      >
        <div className={theme === 'dark' ? "flex flex-col p-4 space-y-2 m-2 bg-black rounded-md" : "flex flex-col p-4 space-y-2 m-2 bg-gray-100 rounded-md"}>
          {menuItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
